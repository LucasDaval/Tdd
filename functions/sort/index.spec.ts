...
describe('sort files in a folder ', () => {
    ...
    it('should sort by alphabetic order is alpha given', () => {
        ...
        expect(folder1.sort(folder1, 'alpha')).toEqual([{name: 'Adoc', signed: false},{name: 'Bdoc', signed: false},{name: 'Cdoc', signed: false}])
    })
    it('should sort by inverse of alphabetic order is !alpha given', () => {
        ...
        expect(folder2.sort(folder2, '!alpha')).toEqual([{name: 'Cdoc', signed: false},{name: 'Bdoc', signed: false},{name: 'Adoc', signed: false}])
    })
})